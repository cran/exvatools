...
author: 
  - name: Enrique Feás
    affiliation: Elcano Royal Institute
date: January, 2026
...
the OECD ICIO tables (2025, with data up to 2022), we must first download to
our computer the source file "2017-2022_EXT.zip" (136 MB) from the
...
and Mexico. We would just have to define the variable. If we include in the
vector the name of the group, it will appear as such in the results.
...
We will use the `wwz` decomposition and extract the domestic value added 
in intermediates (`DVA_INT`) from `LATAM` to `USA`. Note that, had not we put
a name, the exporter will just appear in the results as `"ESP|MEX"`
